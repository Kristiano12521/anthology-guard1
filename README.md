...
| `tools/fill_reference_addons.py <MO2> [--dry-run] [--profile] [--prune]` | наполняет `reference/addons/` из включённых модов MO2 (`modlist.txt` профиля). Тот же набор каталогов. `--prune` без `--yes` ничего не удаляет |
