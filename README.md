...
1. **Наполнить `reference/`.** `python3 tools/fill_reference.py <папка игры>`, затем `python3 tools/fill_reference_addons.py <папка MO2>`. Подробно: [`docs/setup.md`](docs/setup.md).
...
| `tools/check_installed.py <MO2> [--no-mtime]` | сверка установленных пакетов (`BUILD_INFO.txt` в `mods/`) с `addon/`: устарел / актуален / не установлен. SKIP/SEPARATE не считаются пропавшими. В CI и после clone (`--no-mtime`) mtime не сравнивается |
