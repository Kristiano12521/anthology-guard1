...
блок FATAL ERROR, Lua-кадры, осмысленные строки перед падением и повторяющиеся
предупреждения, после чего в чат уходит карточка, а не весь файл.
...
class LogReport:
    ...
    def parse(self, context_lines: int) -> None:
        ...
        with self.path.open("rb") as handle:
            for raw in handle:
                ...
                if stripped[:1] in WARNING_PREFIXES and len(stripped) > 2:
    ...
    def to_markdown(self, max_warnings: int, warnings_only: bool) -> str:
        crash_class, hints = self.classify()
        ...
        if self.crashed and not warnings_only:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        text = report.to_markdown(args.max_warnings, args.warnings_only)
