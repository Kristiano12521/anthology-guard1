...
def write_changelog(count: int) -> str:
    return (
        ...
        "fix_taskboard_sync 1.0.3 accept под pcall без currently_processed.\n"
        "- AIO 1.0.2: fix_create_squad_nil_smart (sim_board CTD), "
        "fix_rvr_active_storages_nil (stash CTD), fix_drx_enemy_task_gate "
        "(маскировка на enemy-task); сняты лишние z-префиксы у event-based.\n"
        ...
        "карантин снимается set_switch_online(true); unsafe не трогаем. "
        "Снимать AIO: загрузка с пакетом → меню (лог exit quarantine freed=…) "
        "→ выключить слот.\n"
        "- fix_matches_campfire_softlock 1.0.1: FDDA анимация спичек у костра — "
        "watchdog 8 с снимает disable_input, если cam-callback не сработал "
        "(soft-lock «ничего не могу делать»).\n"
        "- Monkey-patch re-wrap: orig_* больше не обнуляется на "
        "actor_on_first_update (CTD как fix_sim_mechanic_trade). "
        "fix_sim_mechanic_trade, fix_arti_frames_nil, "
        "fix_attribute_assistent, fix_dotmarks_dropped_weapon, "
        "fix_dynamic_armor_visuals_nil, fix_pda_buyinfo_gui, "
        "fix_qaw_ammo_nil, fix_smart_terrain_state_write, "
        "fix_talents_pda_respec, fix_wtf_assault_instacomplete, "
