...
- Линтер: FORK-001 больше не шумит на `mon_sleep.script`, `guaranteed_loot.script`, `aes_crow_spawner.ltx` — эти full-file подкладывает `pack_bhs.py` из вендора при сборке (`VENDOR_FULL_FILES`), в overlay их нет намеренно.
