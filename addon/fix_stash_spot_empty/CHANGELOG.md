...
- `physic_object_on_use_callback`: сравнение актёра через `who:id() == db.actor:id()` в `pcall`, не `who ~= db.actor`.

**Причина**

У `game_object` нет оператора `__eq`. Открытие тайника → callback → FATAL `No such operator [__eq] defined in class [game_object]` (лог `xray_mg9000 (1).log`).

**Не затронуто**

- Логика снятия спотов, repair-проход, `actor_on_item_take_from_box`

**Проверено**

- lint: после правки
- в игре: не прогонялось; ожидание — открытие тайника без FATAL, presence `loaded v1.0.1`
