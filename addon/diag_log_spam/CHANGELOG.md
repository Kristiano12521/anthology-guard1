...
**Требования:** Anomaly 1.5.3 / Anthology 2.1 / Modded Exes MT; после `install_wrappers` — сводка и «первый TRACE» за текущую сессию, не накопительно.

**Удаление**

- Отключить слот в MO2; новая игра не нужна.
- Сейв не меняется. Без обёрток `alife_release` / `get_object_by_id` снова возможен исходный лог-спам или CTD на nil-release — это не след отключения.
