...
**Требования:** Anomaly 1.5.3 / Anthology 2.1 / Modded Exes MT; В MO2 ниже R ниже базовой линии (~80 за сессию, объекты `grenade_rgn_impact_explosion` / `grenade.
...
- `gamedata/scripts/zzz_fix_minigun_dead_parent.script` → `fix_minigun_dead_parent.script` (префикс `zzz_` снят); убран `-- load-order`.

**Причина**

Мод WITHDRAWN: `install()` ничего не патчит. Патч по событию / no-op — алфавитное место не нужно.

**Не затронуто**

- статус WITHDRAWN, текст причины в логе
