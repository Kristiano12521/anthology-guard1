...
**Требования:** Anomaly 1.5.3 / Anthology 2.1 / Modded Exes MT; В MO2: как 1 после wrap модуля пишет обёртку в `ui_options.
...
- `on_unregister` вешается на класс из `class_registrator`: `se_monster.se_monster` и `se_stalker.se_stalker`. Если метода на вложенном классе нет, остаётся запасной путь — метод прямо на таблице скрипта.

**Причина**

Лог mg9000 2026-09-26: `se_monster.on_unregister not found` и то же для `se_stalker`, остальные wrap'ы этого мода вставали. Движок регистрирует серверный класс как `se_monster.se_monster` / `se_stalker.se_stalker`. Метод лежит на вложенном классе, не на таблице скрипта, куда смотрел `rawget(_G, "se_monster").on_unregister`.

**Не затронуто**

- Остальные гарды (`item_knife`, semenov, `clear_dead`, vid_mode и прочие)
- Сейвы

**Проверено**

- `tools/lint_addon.py fix_nil_crash_guards`
- В игре ещё нет
