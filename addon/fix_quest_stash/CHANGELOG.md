...
**Требования:** Anomaly 1.5.3 / Anthology 2.1 / Modded Exes MT; В MO2 ниже сборки После миграции stash_type уже в формате Anthology и остаётся в сейве — плюс.

**Удаление**

- Загрузите сейв, дождитесь миграции типов тайников (лог migrate, ~секунды), сохранитесь — затем отключайте.
- После миграции stash_type уже в формате Anthology и остаётся в сейве — плюс. Отключать до миграции на legacy-сейве нельзя: задания на тайник снова залипнут.
...
- Комментарии и сообщения лога переведены на русский.

**Причина**

1. Слепой обход 1–7 + `alife_release` забирал чужой leftover `drx_sl_quest_item_N` (завершённое/чужое задание не в `task_info`), подменяя его каноническим КПК активного remap-задания.
2. После `try_spawn_treasure` кэш становится `true`, а не строкой: `cache_pending` пропускал, recover давал второй КПК на актёра и пропускал этап лута ящика.

**Не затронуто**

- Миграция `stash_type` / `stash_type_legacy`, перепись pending-строки, DLTX секций 1001–1038 и blacklist.
- Конвертация, когда у актёра реально лежит КПК с типом из `stash_type_legacy`.
...
- lint: `python tools/lint_addon.py fix_quest_stash`
- В игре: не прогонялось. Ожидание: нет `converted_legacy_item` без `stash_type_legacy`; нет `recovered_consumed_cache` при открытом кэше; в логе возможны `пропуск recover ... кэш открыт`.
