...
- Отключить слот в MO2 после того, как в логе был cleared fixed_bones / already clear; новая игра не нужна.
- Правка пакета (fixed_bones) остаётся в сейве навсегда — это плюс: баг кости не вернётся. Runtime-флаги только на сессию. Действий перед снятием нет.
...
- код в `addon/fix_kupol_wrong_bone/` (остаётся для отдельной установки / сравнения)
- `fix_ph_door_rx_reload` в AIO: в ядре 1.0.1, у нас 1.0.2

## [1.0.2] — 2026-08-31

**Изменено**

- `gamedata/scripts/fix_kupol_wrong_bone.script` — перед запасным `for id = 1, MAX_ALIFE_ID` комментарий `-- alife-scan: запасной путь, …` (LUA-008).
...
- отбор цели, `try_fix_object`, callback'и, кэш id
