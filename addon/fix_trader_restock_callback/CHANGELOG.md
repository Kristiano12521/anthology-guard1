...
**Требования:** Anomaly 1.5.3 / Anthology 2.1 / Modded Exes MT; В MO2 не требует слота относительно Campfires/BHS: обёртка ставится в `on_game_start` после top-level патча BHS после MT reload.
...
- `gamedata/scripts/aaa_fix_trader_restock_callback.script` — повторный `declare()` в `on_game_start` остаётся; `printf` только если статус отличается от попытки на загрузке модуля.

**Причина**

`trader_on_restock added v1.0.2` печаталось дважды: файл и `on_game_start`. Вторая попытка — страховка, в логе выглядела дублем.

**Как исправлено**

Менялся только вывод в лог, не поведение Add / Send wrap / заглушки позднего Add.

**Не затронуто**

- wrap `timed_update`, `callback_add` no-op, сохраняемое состояние
...
- В игре: сессия 31.08.2026, 1.0.2 — callback объявлен, Send wrap установлен, поздний Add от BHS заглушён без traceback. 1.0.3 меняет только повторную строку в логе.
