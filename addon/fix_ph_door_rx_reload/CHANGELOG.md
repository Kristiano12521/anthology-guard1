...
**Требования:** Anomaly 1.5.3 / Anthology 2.1 / Modded Exes MT; В MO2 ниже сборки после MT reload; uninstall возвращает orig только если слот ещё наш.
...
- `meta.ini`: `core_supersedes=1` — в ядре Anthology лежит 1.0.1, у нас 1.0.2; линтер не орёт LUA-001 на намеренную замену устаревшей копии из `reference/anthology/`.
...
- Только логирование: безусловная presence-строка при загрузке; `printf` с причиной при раннем выходе из `install_door()` / `install_rx()`.

**Не затронуто**

- Обёртки ph_door и rx_ai, колбэки.
