...
- `gamedata/scripts/z_fix_quest_stash_hint.script` -> `fix_quest_stash_hint.script` (префикс `z_` снят); убран `-- load-order`.
...
- Логика спота после set_random_stash, фильтр anom inv_box, fallback
