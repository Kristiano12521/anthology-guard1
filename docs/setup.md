...
- `--prune` удаляет папки в `addons/`, которых нет среди включённых модов. По умолчанию выключено; без `--yes` печатает список к удалению и выходит, ничего не копируя и не удаляя.
