...
`--dry-run` печатает архивы и пути, ничего не пишет. Повторный запуск не дублирует файлы: совпавший байт в байт пропускается, лишние уже лежащие файлы не удаляются. В конце — сводка «По архивам:» с числом файлов из каждого архива.

Что делает скрипт:

- Ищет архивы `.db`/`.dbN` и `.xdb`/`.xdbN` (Anomaly часто кладёт ваниль в `configs.xdb0`, не `.db0`) в `<игра>/db` и вложенных папках.
...
- Куда класть: `anthology` в относительном пути от `db/` → `reference/anthology/`; первый каталог `mods/` → `reference/builtin/`; иначе → `reference/anomaly/`. Один путь в разных слоях лежит раздельно (перекрытие — у `refindex`); внутри слоя поздний архив перезаписывает ранний.
