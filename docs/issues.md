...
- **2026-09-13** · `wpo_loot_throttle.script:19: attempt to call global 'IsTimeEventActive' (a nil value)` · нет **`IsTimeEventActive`** · сторонний `wpo_loot_throttle` (в `reference/` файла нет; в `[WPN][2][WPO]…` есть только `wpo_loot.script`) · **функции в сборке нет вовсе** (`refindex` / `lua_help`: 0 определений и вызовов; похожего `Is*` нет, семейство только `CreateTimeEvent` / `RemoveTimeEvent` / `ResetTimeEvent`) · **нашего фикса нет** (не полифиллить чужой API; к автору throttle / снять мод) · [2026-09-13_xray_mg9000-1.log.md](../logs/cards/2026-09-13_xray_mg9000-1.log.md) · pitfalls — нет

- **2026-09-13** · `aaaa_script_fixes_mp.script:936: attempt to call method 'force_set_restrictor_type' (a nil value)` · нет метода **`force_set_restrictor_type`** · ядро Anthology `aaaa_script_fixes_mp` (у тестера xrCore **10037**; в текущем эталоне `reference/anomaly/scripts/aaaa_script_fixes_mp.script` этого вызова нет — строка 936 другая) · **метода в сборке нет**; похожий — **`set_restrictor_type`** (вызовы в `bind_anomaly_field.script`; в `lua_help` тоже не описан) · **нашего фикса нет** (не подменять `force_*` → `set_*` вслепую; обновить `aaaa` / exes до ревизии без вызова) · [2026-09-13_xray_Никита.log.md](../logs/cards/2026-09-13_xray_Никита.log.md) · pitfalls — нет
