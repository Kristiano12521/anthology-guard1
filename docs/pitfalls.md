...
Для `.script` имя файла определяет порядок выполнения: движок грузит все скрипты и сортирует их по имени.

**Критерий префикса:** `z`/`aaa` нужен только когда `install()` / monkey-patch выполняется на загрузке файла и должен попасть до или после конкретного чужого скрипта. Если патч ставится по событию (`on_game_start`, `actor_on_first_update`, `RegisterScriptCallback`) — к этому моменту все скрипты уже загружены, префикс лишний. Длинный хвост (`zzzzzz_`) оправдан только попаданием между двумя конкретными именами или после вендора с таким же хвостом; «просто после ванили» достаточно `z_`. Одинаковый префикс у набора своих overlay сохраняет внутренний порядок по суффиксу имени — разные длины внутри набора меняют этот порядок и требуют отдельной проверки.

Линтер предупреждает ORDER-002; снимается комментарием в первых 10 строках: `-- load-order: после <что>`. Актуальное правило: [`anomaly-core.mdc`](../.cursor/rules/anomaly-core.mdc) (запрет `zzz`/`aaa` только у `.ltx`; у `.script` префикс допустим с этим комментарием и по критерию выше).
...
Несколько Lua-ошибок подряд — разбирай первую по времени; остальные часто каскад. Свежий лог сессии с вылетом обычно в `.bkp` предыдущей сессии при следующем запуске, а не в «текущем» `xray_<user>.log`.
...
- **`pcall(obj:id())` не защищает: аргумент вычисляется до вызова `pcall`.** Рабочая форма — `pcall(function() return obj:id() end)`. Образец: `reference/addons/Anthology_BusyHands_Stability_Fix_v0_6_5/scripts/zzzzzz_anthology_busyhands_stability_fix.script:23`. Это разбор Lua, не замер движка. Источник наблюдения: `fix_stash_id_desync` 1.0.2 писал `pcall(db.actor:id())`.
- **`pcall` не подавляет вывод движка при обращении к члену класса на уничтоженном userdata.** Исключение в Lua ловится, но движок всё равно печатает `cannot access class member …!` и часть стека (`zzz_fix_minigun_dead_parent.script` → `object_alive` / `bullet_parent_alive`). Проверено 01.09.2026, `fix_minigun_dead_parent` 1.0.0: было ~80 `Alive!` от оригинала миниганов, стало 320 при 4200 `dead_parent_skip` под `pcall`. Тот же класс ошибок описан для `:id()` в `fix_stash_id_desync` 1.0.2 (~24700 сообщений). **Не вызывать** `:alive()`, `:id()` и другие методы `CScriptGameObject` на ссылке, если объект может быть уничтожен.
- **`gameobjects_registry[id]` не гарантирует валидный handle.** Реестр хранит id онлайн-объектов (`callbacks_gameobject.script:28-35`), но `level.object_by_id(id)` на тот же id может вернуть протухший userdata — реестр не отличает «id ещё в таблице» от «ссылка пригодна для `:alive()` / `:id()`». Проверено 01.09.2026, `fix_minigun_dead_parent` 1.1.0: `parent_id` есть в `gameobjects_registry`, обёртка пропускает вызов, оригинал R.A.K minigun (`register_npc_minigun_bullet`, стр. 615) всё равно падает на `parent_obj:alive()` — ~80 `Alive!`/сессию, как без мода. Ни `pcall`, ни проверка реестра не заменяют защиту в upstream-скрипте между `safe_level_object_by_id` и вызовом метода.
...
- **DLTX-оператор `!` не удаляет ключи, содержащие двоеточие в имени.** `![item_combination]` + `!magnifier:e0t2` ключ не снимает. Проверено 04.09.2026, Anthology 2.1 / Modded Exes MT, `-dbg`: `itms_manager.ini_craft:line_count("item_combination")` → `ic_keys=10`; `line_exist(...,"magnifier:e0t2")` → `true`. В `reference/` на 11863 `.ltx` нет ни одного `!ключ:с:двоеточием`; соседний хотфикс R.A.K Balance (`mod_craft_fillable_canteens_mod.ltx`) такие ключи не удаляет, а комментирует у себя. Источник: `fix_item_combination_magnifiers` 1.0.0 → 1.1.0 (WITHDRAWN).
