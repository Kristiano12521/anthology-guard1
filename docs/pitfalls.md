...
- **Полный проход id `1..65534` вместо `alife():iterate_objects`.** Источники: `fix_aver_darkvalley` 1.0.1, `fix_kupol_wrong_bone` 1.0.2 (запасной цикл, если `iterate_objects` нет или `pcall` упал). Ловится LUA-008; снимается комментарием `-- alife-scan: запасной путь, <причина>` в трёх строках перед циклом.
- **`pcall(obj:id())` не оборачивает вызов.** Аргумент вычисляется до `pcall` — это Lua, не специфика X-Ray. В эталоне рабочий гард — `pcall(function() return obj:id() end)`: `reference/addons/Anthology_BusyHands_Stability_Fix_v0_6_5/scripts/zzzzzz_anthology_busyhands_stability_fix.script:23` (комментарий про destroyed userdata и обёртка `:id()`). Исходное наблюдение: `fix_stash_id_desync` 1.0.2 писал `pcall(db.actor:id())`. Про «C++ member error всё равно в лог» — см. перепроверку ниже.
...
- **`pcall` не глушит C++ member error** на destroyed `CScriptGameObject` (`cannot access class member …::ID!`) — в смысле «движок всё равно пишет traceback в лог». Искал строку `cannot access class member` по `reference/**/*.script` — 0 попаданий. `script_game_object` / luabind в эталоне нет. Скрипты эталона как раз ставят `pcall(function() return obj:id() end)` и по `not ok` считают объект мёртвым (BHS, см. выше). Отсекать по данным без `:id()` по-прежнему разумный совет, но «pcall не ловит C++» как факт движка из Lua не следует. Проверка: `run_string` с `-dbg` на заведомо destroyed userdata — `pcall(function() return obj:id() end)` и смотреть, есть ли строка в логе при `ok == false`. Источник исходной формулировки: `fix_stash_id_desync` 1.0.2.
- **DLTX `mod_*` только при `bIsRootFile`; патч файла из `#include` не грузится.** Искал `Xr_ini.cpp`, `bIsRootFile`, `LTXLoad` — в `reference/` этих файлов нет. Цепочка include есть: `reference/anomaly/configs/system.ltx:10` → `reference/anomaly/configs/items/item_base.ltx:12` → `reference/anomaly/configs/items/items/base.ltx:7` `#include "items_*.ltx"`. `ini_file` / `ini_file_ex` с `items_quest` — 0 попаданий. Контраст: `treasure_manager.ltx` открывается как свой корень (`reference/anomaly/scripts/treasure_manager.script:29`) — `mod_treasure_manager_*` в том же моде `fix_quest_stash` не тот случай. В сборке полно `mod_*` рядом с файлами, которые Lua открывает через `ini_file()` (trade, `animations_settings.ltx`) — это не опровергает правило про include, но и не доказывает `bIsRootFile`. Проверка: исходник Modded Exes `Xr_ini.cpp`, либо в игре только `mod_items_quest_*` без `mod_system_*` и `ini_sys:section_exist("drx_sl_quest_item_1014")`. Источник: `fix_quest_stash` 1.0.1 → 1.0.2 (в CHANGELOG обоих — «в игре не прогонялось»).
