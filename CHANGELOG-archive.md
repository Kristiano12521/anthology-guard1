...
Записи корневого `CHANGELOG.md` старше месяца от даты архивации.
Актуальное — в [`CHANGELOG.md`](CHANGELOG.md).

Первый перенос 2026-09-19: в основном файле оставлены записи с сентября
(`0.1.46`…); ниже — август `0.1.0`–`0.1.45`. Весь репозиторий моложе
календарного месяца, поэтому отсечка «до сентября», а не «строго >30 дней».

`tools/check_changelog_tools.py` смотрит только `CHANGELOG.md` (добавленные
строки диффа) — этот файл в сверку не входит.
...
## [0.1.40] — xraylog: «Мои моды» всегда в карточке

Секция **«Мои моды»** теперь вставляется в карточку безусловно — сразу после шапки, до FATAL и нефатальных ошибок. При `--errors-only` / `--warnings-only` строки загрузки из лога не теряются: скан идёт по сырым строкам, а не по отфильтрованным секциям. В `--errors-only` убран блок «Последние строки лога». `LOAD_HINT` дополнен: `added`, `wrap installed` (как у `fix_trader_restock_callback`). Тесты: мод виден при `--errors-only`, отсутствующий мод в блоке «Не появились в логе».
...
## [0.1.30] — pitfalls: pcall Lua-порядок подтверждён, STACK TRACEBACK не замерили

В [`docs/pitfalls.md`](docs/pitfalls.md) §16: `pcall(obj:id())` не защищает (аргумент вычисляется до вызова) — в основном разделе, образец BHS v0_6_5. В «требует перепроверки» осталось: пишет ли движок `STACK TRACEBACK` при пойманной `pcall`-ошибке. Три неудачных замера 31.08.2026 (две команды в одном вводе; `_tmp` не переживает смену уровня; `db` пересоздаётся).
...
В таблицу README добавлены `to_cp1251.ps1`, `check_encoding.ps1`, `dds_tool.ps1`, `draw_cmo_unique_icons.ps1`, `pack_bhs.py`. В `anomaly-core.mdc` — одна строка: перекодировку, проверку кодировки и DDS брать из готовых скриптов, не писать одноразовых.
